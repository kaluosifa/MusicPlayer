...
{
	static HWND hListBox;						// 列表盒句柄
	static HWND hMusicPlay;						// 播放按钮句柄
	static HWND hMusicPauseContinue;			// 暂停继续按钮句柄
	...
	case WM_CREATE:
		{
			hListBox = CreateWindow(L"LISTBOX", "", LBS_EXTENDEDSEL,
				0, 0, 600, 400, hWnd, (HMENU)LISTBOX_1, hInst, lParam);
			hMusicPlay = CreateWindow(L"BUTTON", L"播放", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_NOTIFY,
				20, 420, 40, 20, hWnd, (HMENU)BUTTON_PLAY, hInst, lParam);
			hMusicPauseContinue = CreateWindow(L"BUTTON", L"暂停", 
				WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON | BS_NOTIFY,
				60, 420, 40, 20, hWnd, (HMENU)BUTTON_PAUSE, hInst, lParam);
